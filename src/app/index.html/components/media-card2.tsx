...
export default function MediaCard2({ d, cids, styles }: { d: MediaCard2Data; cids: string[]; styles: MediaCard2Styles }) {
  return (
    <div data-cid={cids[0]} className={cn("w-200 h-112.5 block relative max-w-200 rounded-lg shrink-0 aspect-video bg-cover [background-position:50%_50%] shadow-[var(--clr-13)_0px_15px_50px_0px] origin-[400px_225px] max-md:w-[217.7px] max-md:h-[7.65rem] max-md:origin-[108.844px_61.2188px] md:max-lg:w-[30.8rem] md:max-lg:h-[17.325rem] md:max-lg:origin-[246.398px_138.594px]", styles.className)} aria-label={d.ariaLabel} role="group">
      <div data-cid={cids[1]} className="group h-112.5 border border-solid border-surface hover:border-primary transition-colors duration-300 block relative rounded-sm overflow-hidden bg-color-002 shadow-[var(--clr-2)_0px_10px_30px_0px,var(--border)_0px_0px_15px_0px] cursor-pointer max-md:h-[7.65rem] md:max-lg:h-[17.325rem]">
        <div data-cid={cids[2]} className="w-full h-112 block relative rounded-lg overflow-hidden aspect-[2/3] max-md:h-[7.525rem] md:max-lg:h-[17.2rem]">
          <img data-cid={cids[3]} className="w-199.5 h-112 block relative z-1 max-w-full overflow-clip object-contain origin-[399px_224px] transition-transform duration-500 group-hover:scale-110 max-md:w-54 max-md:h-30 max-md:origin-[107.844px_60.2188px] md:max-lg:w-[30.6875rem] md:max-lg:h-[17.1875rem] md:max-lg:origin-[245.398px_137.594px]" alt={d.alt} id={d.id} src={d.imgSrc} />
          {" "}
          <video data-cid={cids[4]} className="w-199.5 h-112 block absolute top-0 left-0 z-1 opacity-0 max-w-full overflow-clip object-cover pointer-events-none" muted playsInline preload="none">
            {" "}
          </video>
          {" "}
          <div data-cid={cids[5]} className="w-199.5 h-112 flex absolute top-0 left-0 z-2 opacity-0 group-hover:opacity-100 transition-opacity duration-300 p-6 flex-col justify-end" style={{ backgroundImage: "linear-gradient(to top, var(--clr-14) 0%, var(--clr-15) 40%, var(--clr-4) 100%)" }}>
      ...
      <div data-cid={cids[17]} className="w-200 h-112.5 block absolute top-0 left-0 z-10 opacity-0 pointer-events-none" style={{ backgroundImage: "linear-gradient(to right, var(--clr-13), var(--clr-4))" }} />    </div>
  );
}
